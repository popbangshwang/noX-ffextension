function blockTextContent() {
  ...
  function blockWords(node) {
    ...
    if (node.style && node.style.filter === "blur(20px)") return;
      ...
      for (const word of words) {
        if (new RegExp(`\\b${word}\\b`, "i").test(node.textContent)) {
          ...
          while (parent && parent !== document.body) {
            ...
            if (parent.style && parent.style.filter === "blur(20px)") return;
  ...
  images.forEach(img => {
    ...
    if (window.scannedImages.has(img) || (img.style && img.style.filter === "blur(20px)")) return;
  ...
  resultLinks.forEach(link => {
    ...
    if (window.scannedImages.has(link) || (link.style && link.style.filter === "blur(20px)")) return;
